...
import { X, Share2, Heart, Star, ChevronDown, Check, MousePointer2, Settings2, User, Shirt, Box, Image as ImageIcon } from 'lucide-react';
import { GeneratedImage } from '../types';
import { MockupViewer } from './mockup/MockupViewer';
import { OrderWizard } from '@/features/order';
...
const MockupModal: React.FC<MockupModalProps> = ({ image, onClose }) => {
   ...
   const [viewMode, setViewMode] = useState<'2d' | '3d'>('2d'); // Toggle between 2D and 3D view
   const [showOrderWizard, setShowOrderWizard] = useState(false); // Order wizard visibility
         ...
         <div className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            >
               ...
               <div className="flex-1 flex flex-col md:flex-row overflow-hidden pt-6">
                  ...
                  <div className={`flex-1 bg-[#09090b] relative flex flex-col p-4 md:p-8 border-b md:border-b-0 md:border-r border-white/5 overflow-y-auto custom-scrollbar transition-all ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}
                  ...
                  >
                     ...
                     <div className="flex gap-2 mb-4 self-center">
                        <button
                           ...
                           className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${viewMode === '2d'
                        ...
                        <button
                           ...
                           className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${viewMode === '3d'
                              ? 'bg-indigo-600 text-white'
                              : 'bg-white/5 text-zinc-400 hover:bg-white/10'
                              }`}
                        >
                     ...
                     <div className="flex-1 flex items-center justify-center">
                        {viewMode === '2d' ? (
                           /* 2D Static Mockup */
                           <div className="relative w-full max-w-md aspect-[3/4] bg-[#18181b] rounded-2xl overflow-hidden shadow-2xl border border-white/5">
                              <img
                           ...
                           <div className="w-full max-w-lg h-[500px]">
                              <MockupViewer imageUrl={image.url} />
                           </div>
                        )}
                     </div>
                  </div>
                     ...
                     <div className="p-8 md:p-10 space-y-8">
                        ...
                        <div className="flex gap-3">
                           <button
                              ...
                              className="flex-1 bg-indigo-600 text-white h-12 rounded-xl font-semibold text-base hover:bg-indigo-500 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20"
                           >
                              Order Now
                           </button>
