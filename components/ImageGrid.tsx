...
import { Check, Share2, Download, Maximize2, Wand2, Image as ImageIcon, MoreVertical, Repeat } from 'lucide-react';
...
const ImageGrid: React.FC<ImageGridProps> = ({ images, onImageClick, onMockupClick, onBookmarkClick, onEditClick }) => {
  const [selectedForMockup, setSelectedForMockup] = useState<Set<number>>(new Set());
  const [activeMenuId, setActiveMenuId] = useState<number | null>(null);
  const menuButtonRefs = React.useRef<Map<number, React.RefObject<HTMLButtonElement>>>(new Map());
  ...
  const getMenuButtonRef = (imageId: number) => {
          ...
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
            ...
            <div className="absolute top-3 left-3 pointer-events-auto">
              <button
                ...
                className="mockup-select-btn w-9 h-9 rounded-full bg-black/60 backdrop-blur-sm text-white hover:bg-indigo-600 flex items-center justify-center transition-all shadow-lg z-20 pointer-events-auto"
