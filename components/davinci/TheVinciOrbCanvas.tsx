...
export const TheVinciOrb: React.FC<TheVinciOrbProps> = ({
    ...
    // Fallback UI for when WebGL is not available or still loading
    const FallbackOrb = (
    ...
    // If WebGL is not supported, show only the CSS fallback
    ...
    // If still checking, show loading fallback
        ...
        >
            ...
            >
                <Canvas
                    ...
                    dpr={[1, 1.5]}
                    onCreated={(state) => {
                        // Canvas successfully created, silence any lingering warnings
                        if (!state.gl) {
                            console.warn('TheVinciOrb: WebGL renderer not available');
                        }
                    }}
                    style={{
